...
xl_age = pd.read_excel(FILE, sheet_name="Table 5", header=None)

# Constants for sheet layout (0-based)
HEADER_ROW = 3           # Excel row 4: religion labels
...
religion_groups = {c: xl_age.iloc[HEADER_ROW, c] for c in percent_cols}
...
for col in percent_cols:
    group = str(religion_groups[col]).strip()
...
df.to_sql(
    "vaccine_hesitancy_religion",
    engine,
    if_exists="replace",
    index=False,
    schema="public"
)
print(f"✅ Loaded {len(df)} rows into vaccine_hesitancy_religion on Supabase")