...
xl_age = pd.read_excel(FILE, sheet_name="Table 1a", header=None)

# Constants for sheet layout (0-based)
HEADER_ROW = 3           # Excel row 4: age band labels
...
age_bands = {c: xl_age.iloc[HEADER_ROW, c] for c in percent_cols}
...
df.to_sql(
    "vaccine_hesitancy_age",
    engine,
    if_exists="replace",
    index=False,
    schema="public"
)
print(f"✅ Loaded {len(df)} rows into vaccine_hesitancy_age on Supabase")