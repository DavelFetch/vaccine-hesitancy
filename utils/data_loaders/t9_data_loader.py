...
xl_age = pd.read_excel(FILE, sheet_name="Table 9", header=None)

# Constants for sheet layout (0-based)
HEADER_ROW = 3           # Excel row 4: Health general condition
...
health_general_condition = {c: xl_age.iloc[HEADER_ROW, c] for c in percent_cols}
...
for col in percent_cols:
    condition = str(health_general_condition[col]).strip()
    ...
    for measure, pct, lcl, ucl in zip(measures, pcts, lcls, ucls):
        records.append({
            ...
            "health_general_condition": condition,
            "measure": measure,
            "percent": pct,
            "lcl": lcl,
            "ucl": ucl,
...
df.to_sql(
    "vaccine_hesitancy_health_general_condition",
    engine,
    if_exists="replace",
    index=False,
    schema="public"
)
print(f"✅ Loaded {len(df)} rows into vaccine_hesitancy_health_general_condition on Supabase")