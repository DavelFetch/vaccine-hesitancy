...
xl_age = pd.read_excel(FILE, sheet_name="Table 8", header=None)

# Constants for sheet layout (0-based)
HEADER_ROW = 3           # Excel row 4: Health condition
...
health_condition = {c: xl_age.iloc[HEADER_ROW, c] for c in percent_cols}
...
for col in percent_cols:
    condition = str(health_condition[col]).strip()
...
df.to_sql(
    "vaccine_hesitancy_health_condition",
    engine,
    if_exists="replace",
    index=False,
    schema="public"
)
print(f"✅ Loaded {len(df)} rows into vaccine_hesitancy_health_condition on Supabase")