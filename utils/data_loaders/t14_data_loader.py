...
xl_age = pd.read_excel(FILE, sheet_name="Table 14", header=None)

# Constants for sheet layout (0-based)
HEADER_ROW = 3           # Excel row 4: Household type 
...
household_type = {c: xl_age.iloc[HEADER_ROW, c] for c in percent_cols}
...
for col in percent_cols:
    htype = str(household_type[col]).strip()
...
df.to_sql(
    "vaccine_hesitancy_household_type",
    engine,
    if_exists="replace",
    index=False,
    schema="public"
)
print(f"✅ Loaded {len(df)} rows into vaccine_hesitancy_household_type on Supabase")